...
public class PromptGenerator {
  ...
  public static String getEvoLLMAgentPrompt(String testSuite,
    ...
    return "The task I am currently working on is to perform differential testing on two versions" +
            ...
            "suite is not comprehensive enough to detect the regression error in the buggy " +
            "version. Hence, your task is to modify the provided test suite such that the goals " +
            "of this task (mentioned earlier) have been fulfilled. Here is the test suite " +
            "that Evosuite generated: \n" + testSuite + "\n\nAdditionally, I have also " +
            "identified some suspicious lines in the working and buggy versions of the method. " +
            "The suspicious lines for the working version is as follows:\n" +
            workingSuspiciousLines + "\n\n The suspicious lines for the regression version is as " +
            "follows:\n" + regressionSuspiciousLines + "\n\nFinally, here is the coverage " +
            "information of the test suite generated by Evosuite on the working version. " +
            "The uncovered suspicious lines have also been included below:\n" + coverage +
            "\n\nUsing the suspicious lines and coverage information provided, your task is " +
            "to modify the generated test suite such that when it is run on the working version, " +
            "all test cases pass and when run on the regression version at least one test case " +
            "fails. Since this can be quite a challenging task, I first want you to decompose " +
            "this task into smaller subtasks that you think are essential to solving this " +
            "problem. AFter you have identified these subtasks, we will work on each of them one " +
            "by one, breaking each subtask down into even smaller subtask if required. We will " +
            "continue this process until we have a set of steps that oyu think you can implement " +
            "to solve the problem. You can start by listing the subtasks required to solve this " +
            "problem.";
  }
  ...
  public static String getEvoFullImplementationPrompt(String testFileName) {
    return "Based on our previous discussion where you broke down the task of differential " +
            "testing into various smaller subtasks and provided the implementation for each " +
            "subtask wherever possible, can you combine all the implementations you have " +
            "provided for all implementable subtasks and come up with a complete and compilable " +
            "implementation that would solve the differential testing problem? Take note that " +
            ...
            "You should return the entire test suite, including all import statements, " +
            "the existing tests in the class (do not truncate them with comments) and the " +
            "newly added test cases as your complete implementation. Lastly, you do not have to " +
            "provide any package statement and you are to name the implementation class \"" +
            testFileName + "\"";
  }
}
